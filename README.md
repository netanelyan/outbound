...
call, and its several hundred gorse bushes are one instanced mesh. The textures
— tarmac, markings, ground mottle, building facades, roof tiles, the sky — are
all drawn into canvases at load time. There are no image files anywhere in the
project, and the favicon is an inline SVG in the `<head>`.
